...
namespace WebApp.Controllers
{
    public class LoginController : Controller
    {
        ...
        public ActionResult CheckLogin()
        {
            ...
            var userInfo = userInfoService.LoadEntitie(u => u.UName == userName && u.UPwd == userPwd).FirstOrDefault();//对用户名密码进行过滤.
            if (userInfo == null)
            {
            ...
            else
            {
                ...
                Response.Cookies["sessionId"].Value = sessionId;//然后将自创的sessionId以Cookie的形式返回到浏览器，存储到浏览器端的内存中。
                                                                //判断一下用户是否选择了记住我.
                string checkMe = Request["checkMe"];
        ...
        [HttpPost]
        public ActionResult FindPwd(string txtName,string txtMail)
        {
            string txtName1 = txtName;//Request["txtName"];
            string txtMail1 =txtMail;// Request["txtMail"];
            var userInfo = userInfoService.LoadEntitie(u => u.UName == txtName1).FirstOrDefault();
            if (userInfo != null)
            {
                if (txtMail1 == "wangchengwei324@126.com")
                {
        ...
        public void CheckCookieInfo()
        {
            if (Request.Cookies["cp1"] != null && Request.Cookies["cp2"] != null)
            {
                ...
                var userInfo = userInfoService.LoadEntitie(u => u.UName == cookieUserName).FirstOrDefault();
                if (userInfo != null)
                {
